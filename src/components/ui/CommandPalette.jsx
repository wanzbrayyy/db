...
import { Search, Command, ArrowRight, Database, Settings, Code, FileText, User } from 'lucide-react';
...
export default function CommandPalette() {
  ...
  const actions = [
    ...
    { name: 'View Collections', icon: Database, path: '/dashboard/collections' },
    { name: 'Developer Settings', icon: Code, path: '/dashboard/developer' },
    { name: 'Profile & Security', icon: User, path: '/dashboard/profile' },
    { name: 'Read Documentation', icon: FileText, path: '/docs' },
  ];
  
  // Dummy import LayoutDashboard to avoid error in snippet above
  function LayoutDashboard(props) { return <div {...props} /> }
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-black/60 backdrop-blur-sm flex items-start justify-center pt-[20vh]" onClick={() => setIsOpen(false)}>
      <div className="w-full max-w-lg bg-[#09090b] border border-white/10 rounded-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-2 border-t border-white/10 bg-white/[0.02] flex justify-between items-center text-[10px] text-gray-500 px-4">
           <span>Navigate with arrows</span>
           <span>WanzDB Command</span>
        </div>
      </div>
    </div>
  );
